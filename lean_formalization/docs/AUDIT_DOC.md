...
> repository root. The documents `notes/paper_edits.md` and `notes/NOTE_FOR_COAUTHORS.md` are
> named with their `notes/` prefix throughout.
...
parentheses. The build and gate figures come from the transcript in Appendix A, which is the
verbatim output of one command.
...
paragraph follows the external statement audit
`notes/archive/audit_external_statement_2026-09-02.md` (and its response,
`notes/archive/audit_external_statement_response_2026-09-02.md`). Three additions that this
audit asked for are in the tree: the entrywise eigenvector lemma in the paper's signed
coordinate form (section 5), the svdstack clauses of `prop:binarystacksvd_inadmissable` on
the model (7.16), and the Gaussian marginalization behind the MLE objective (7.15).
...
| `thm:theta_est` | `thm_theta_est_gaussian`; `thm_theta_est_general` (noise law `NoiseLaw`) | `ThetaEst.lean` |
...
| `thm:gen_rank_weight_svdstack` | `thm_gen_rank_weight_svdstack_general_r_full_gaussian` | `RankR/GeneralGaussian.lean` |
...
For `prop:singleweight_suboptimality` (`THEOREMS.md` 6.9) the hypothesis `hlaw` is discharged
regime by regime, since the Gaussian discharge of `SingleWeightLaw` alone does not supply it.
The one paper result of 7.5, the removal remark, has no row at all.
...
(`TECHNICAL.md`). The `axiom` keyword occurs nowhere in the development, and
`scripts/check_axioms.sh` is the gate that catches a new one (`scripts/README.md`).
...
`docs/SORRIES.md` is empty, and `scripts/check_sorries.sh` compares that ledger against the
`sorry` sites in the tree. Measured output:
...
Because `docs/SORRIES.md` is empty, a `sorryAx` anywhere in the tree fails the axiom
gate (`THEOREMS.md` 7.1); no declaration carries one.
...
(`TECHNICAL.md`, `notes/SERVER_TODO.md`; backport of commit `2d84602`).
...
`lean/StackedSVD/Vendor/` both return 0 (measured 2026-09-02).
...
On the build of record (commit `416ab9d`, 2026-09-10): 230 modules, 210 ours, 229
reachable, exit 0. The one unreachable module is `Vendor/COLT83/Axioms.lean`, the vendor's
own `#print axioms` driver, which nothing imports on purpose (`scripts/README.md`).
...
| `SpikedModelR` | `RankR/General.lean` | one table with `rk` spikes, orthonormal `U`, `V`; fields `hθnn : ∀ k, 0 ≤ θ k` and `hθanti : StrictAnti θ` |
...
the class and `hθnn` (`0 ≤ θ`) is wider than the paper's positive entries (`THEOREMS.md`
...
(`heteroEdge_of_gaussian`, `THEOREMS.md` 2.2). `heteroLaw_of_gaussian_margin` is a weaker
variant of the same discharge; it needs `MPhet.bSF c w < MPhet.rhoHet θ c w`
(`THEOREMS.md` 2.2).
...
claim. In the paper, Section `sec:model_setup` defines `\pto` as "convergence in
probability of a sequence of random variables"; the Introduction says "We establish convergence in
probability to the limiting forms" and repeats it for the rank-one section. The words
"almost surely" occur twice in the paper, and both are inside the proof of `lem:delocalization`,
not in a theorem statement (read from the paper snapshot, 2026-09-02).
...
paper results, import `RMT.Full` (import headers read 2026-09-03). The Marchenko-Pastur chain that proves the single-table law is `RMT/{MP,
...
For the heteroscedastic chain the file layout shows the split less directly.
`StackSVDWeighted.lean` holds the weighted definitions and the structure `HeteroLaw`, and
`StackSVD/Weighted.lean` holds the Layer 1 consumer `thm_stacksvd_weighted_general`. Three
files of the discharge import
...
such files gate 7 (`scripts/check_core_imports.py`) keeps outside its protected set, EXT
...
consumers before the audit. Result on the build of record: 3 endpoints audited, closures of 1288 (`singleTableLaw_of_gaussian`), 1562 (`heteroLaw_of_gaussian`) and 323 (`heteroEdge_of_gaussian`) project constants, 97 `USES` lines over 75 distinct Layer 1 constants (the structure `HeteroLaw`, its constructor, and definitions and structural lemmas of `StackSVDWeighted.lean`; no theorem that takes a structure as a hypothesis), 0 `FORBIDDEN`, 0 `CONSUMER`, exit 0; the counts are those of the build of record of 2026-09-05 (19:24 to 19:26 EDT; 98 `USES` lines on the later runs, see section 9), the 30 s timing is from the run of 2026-09-03 09:10 EDT.
The heteroscedastic chain is `RMT/Het/` and its rank-`r` twin is `RankR/Het/`
(`THEOREMS.md` 0 glossary, 2.5). The declaration-level check is the stronger statement
...
under some product law with arbitrary marginals (`SVDStack/Gram.lean`); at rank `r` the
general-`r_i` svdstack statements take the same predicate as `hG : m.IndepNoise`. That is
weaker than Gaussian, and it is not the same hypothesis as `JointGaussianNoise`
(`notes/archive/AUDIT_PACK_V4.md` 2.4, C17). `thm_theta_est` reads its two cross-table limits
through `MultiTableModel.ThetaEstLaw` (`THEOREMS.md` 2.6), so no rank-one Layer 1 theorem
keeps `hG : m.JointGaussianNoise` (section 7.1).
...
| `lem:entrywise_conv_eigenvec` | none needed (the hypothesis is entrywise convergence in probability) | `lem_entrywise_conv_eigenvec`, `lem_entrywise_conv_eigenvec_signed` | `SVDStack/Deterministic.lean`, `SVDStack/EntrywiseSigned.lean` | projector form, sign-free, and the paper's signed coordinate form |
...
| `thm:svdstack_weighted` | `thm_svdstack_weighted_gaussian`, `_gaussian_opt`, `thm_svdstack_weighted_gaussian_opt_full`, `thm_svdstack_weighted_paper_gaussian` | `thm_svdstack_weighted`, `_paper`, `_general`, `_inner`, `_zero` | `SVDStack/Weighted.lean`, `SVDStack/Rayleigh.lean` | `_opt_full` is uniform in `w` |
...
| `prop:binarystacksvd_inadmissable` | `prop_binarystacksvd_inadmissable_exists` (the paper's existential form, model built), `prop_binarystacksvd_inadmissable_gaussian` (the five limits on any model of the instance), `stackPerfW_binary_inad_gaussian`, `stackPerfW_opt_inad_gaussian`, `svdstackPerf_inad_gaussian`, `svdstackPerfW_opt_inad_gaussian` | `Scalars.binarystacksvd_inadmissable`, `_ceil` | `Existence.lean`, `RMT/Het/Sup.lean`, `SVDStack/Inad.lean`, `Scalars.lean` | explicit instance, uniform over subsets; the two svdstack clauses on the model; the existential form uses the model `Sat.Rank1.inadModel M` (`k_i = 2i + 1`, `l = 1`) |
| `thm:theta_est` | `thm_theta_est_gaussian`; `thm_theta_est_general` (any fixed noise law with mean 0, variance 1, finite fourth moment; the single-table law of table `i` stays a hypothesis) | `thm_theta_est` | `ThetaEst.lean` | item P proved, not assumed; `ThetaEstLaw` also discharged at `NoiseLaw` (`thetaEstLaw_of_general`) |
| `app:wstacksvd_mle` | deterministic | `mleLogLik_eq`, `mleLogLik_max_iff_mem_topSpace`, `thm_wstacksvd_mle_marginal` | `MLE.lean`, `MLEConverse.lean`, `MLEMarginal/Main.lean` | finite-`N` identity, its converse, and the Gaussian marginalization from the random-effects model |
| `remark:stack_outperform_svd` | `remark_stack_outperform_svd_exists` (model built, every `M ≥ 2`), `remark_stack_outperform_svd_stack`, `_svdstack`, `_svdstack_uniform` | instances of the closed forms | `Existence.lean`, `Remarks.lean`, `RemarksUniform.lean` | explicit rationals; the existential form uses the model `Sat.Rank1.oneModel M` |
| `remark:svd_outperform_stack` | `remark_svd_outperform_stack_exists` (model built, `θ = (√5, 4)`, `c = (1, 38.4)`), `remark_svd_outperform_stack_two`, `_pair_*`, `_three_*` | instances of the closed forms | `Existence.lean`, `Remarks.lean`, `RemarksUniform.lean` | explicit rationals; the existential form uses the model `Sat.Rank1.twoModel` (`n = (5(N+1), 192(N+1))`, `d = 5(N+1)`) |
...
| `thm:gen_rank_weight_svdstack` | `thm_gen_rank_weight_svdstack_general_r_paper_gaussian`, `thm_gen_rank_weight_svdstack_general_r_full_gaussian`, `thm_gen_rank_weight_svdstack_gaussian` | `thm_gen_rank_weight_svdstack`, `_opt`, `_max`, `_paper`, `_general_r`, `_full` | `RankR/GeneralGaussian.lean`, `RankR/Weighted.lean` | `rank B_R = r` in place of `β_ij > 0`; uniform-in-`W` bound |
...
The same construction supplies the models of the paper's three existential
statements (`Existence.lean`, `notes/archive/audit_external_rank1_2026-09-03.md`, findings 1
and 2):
...
1. It compiles: commit `1b56d91`, `lean-local.sh`, exit 0, no errors (measured
   2026-09-02).
...
`θ = 0`, and no `MultiTableModel`-to-`UnalignedModelR` bridge exists to build it
(`SpikedModelR.hθnn` allows `θ = 0` and `SpikedModel.toRankR` maps one rank-one table; the
tree carries no multi-table bridge). The Gaussian discharge
`singleWeightLaw_of_gaussian` builds the structure on every Gaussian
...
at `w = (1, 9/10)` meets `EigSep` numerically (`THEOREMS.md` 8.1), not in Lean.
...
`MultiTableModel.IndepNoise` (`SVDStack/Gram.lean`, `notes/archive/L4_indepnoise.md`): the
Fubini step
...
2.6): item P with a random direction (`‖E_j v̂_i‖² → c_j`) and the
...
`notes/FLAGGED.md` D35).

**7.2 The rank-`r` model class orders the spikes inside a table, and the paper does not.**
`SpikedModelR.hθnn : ∀ k, 0 ≤ θ k` and `hθanti : StrictAnti θ` are model fields. The paper
makes no ordering assumption and assumes distinct entries for svdstack only (both in the text
after `assum:unaligned`); the text after `prop:stacksvd_subspace` says it applies with repeated eigenvalues. So
...
same proof (`THEOREMS.md` 6.3, decision D-A1 in `notes/FLAGGED.md` item 10).
...
worked example before `alg:rank_r_stacksvd`, `θ_1 = (2, 1)` and
`θ_2 = (1, 10)`, is therefore **not** an `UnalignedModelR` with `R_i = 1` (`THEOREMS.md` 6.7).
Whether to keep the class or to add a permutation `R_i` route is open
(`notes/FLAGGED.md` Q16 item 1). The same common order narrows `thm:rank_r_svdstack`:
...
**7.4 The index `ℓ_j` of `alg:rank_r_stacksvd` is the outlier rank.** The paper ranks `λ_jj`
among the `λ_jk` (`eq:stacksvd_lambda`) and asks `λ_jj ≠ λ_jk` (`thm:rank_r_stacksvd`). The
Lean statement stays inside the ordered class, where `ℓ_j = j` (`THEOREMS.md` 6.7).
...
|---|---|---|
| The removal remark | after `thm:gen_rank_weight_svdstack` | not formalized, and wrong in general; deleted in the revised paper (Q8) |

Two more results, `prop:singleweight_suboptimality` and
`prop:gen_rank_stacksvd_singleweight`, have Lean statements
(`THEOREMS.md` 6.8, 6.9, section 5 above). The second is proved at Layer 1 and for Gaussian
noise (`prop_gen_rank_stacksvd_singleweight_gaussian`), and the Gaussian theorems carry no
condition on the weights. The first is proved as an honest implication
from the hypothesis `hlaw` (the single-weight limit on its two-table witness at every
positive weight pair), which the general discharge does not supply because the witness leaves
`EigSep` at extreme weight ratios; `hlaw` is proved on the witness regime by regime, so
`prop_singleweight_suboptimality_gaussian` is unconditional. Neither is counted
among the results with no Lean statement.

The removal remark (now deleted from the paper) failed because removing a component with `β_ij = 0` can break
...
remark after `prop:stacksvd_general`, that the proposition survives `M → ∞` when `‖c‖₁` and
...
`∑_{k<r} ‖specProjTop(A, r) (V e_k)‖²`. Both forms equal `‖V̂ᵀ V‖_F²` on the event
...
`prop_general_rank_unweighted_svdstack_general_frobenius_eig` and the
...
under the model, the Gaussian noise and the regime only. Both `prop_stacksvd_subspace_frobenius`
forms of `RankR/Frobenius.lean` take `UnalignedModel`, the `r_i = 1` model, and
`prop_stacksvd_subspace_general_gaussian` recovers the paper's quantity only on the gap
event (`notes/archive/AUDIT_PACK_V4.md` S12, C16). The surrogate forms stay in the tree; the
Frobenius forms are the paper-literal ones.
...
`θ ≡ 0`, where the paper's display then reads `1 → 0`. The paper excludes no such parameter
point, and the Lean theorems carry the hypothesis `hθ : ∃ i, θ_i ≠ 0` for that reason
(`thm_stacksvd_weighted_gaussian` and `prop_dominance_gaussian`, `RMT/Het/Sup.lean`;
...
- `HeteroLaw.align` is not a strengthened assumption of the paper: it is the weighted random
  ...
  the paper cites at `eq:weighted_norm`, decision D21) placed behind a Layer 1 interface. So
...
**7.15 The MLE result: the identity and the derivation.**
...
matrix (`THEOREMS.md` 5.5, `notes/archive/AUDIT_PACK_V4.md` C15). `thm_wstacksvd_mle_marginal`
(`MLEMarginal/Main.lean`) proves the Gaussian marginalization over `u_i` that produces the
formula: the random-effects
...
(`notes/archive/L5_mle_marginal.md`). The paper's appendix does not name the marginalization
as a standard step; that is a presentation remark, not an error (section 8).
...
same mode (Section `sec:model_setup` and the Introduction); section 3.8 gives the evidence. Nothing is
...
**7.18 One paper lemma has no section of its own.** `lem:noise_projection_concentration`
is proved, not assumed, as `SpikedModel.noise_projection_tendsto`
(`ThetaEst.lean`), and only `thm:theta_est` reads it (`THEOREMS.md` 7.1).

**7.19 Well-formedness hypotheses that remain.** These make an object exist or an index be in
...
| `[NeZero M]`, `hM : 0 < M` | the rank-`r` results, and the 26 rank-one declarations quoted in `THEOREMS.md` that sit inside a section or namespace with `variable [NeZero M]` in force | at least one table, so the stack has rows (`MultiTableModel.stack` reads `(m.tbl 0).v`, `StackSVD.lean:157`). In the rank-one files the instance is a section-level `variable [NeZero M]` (13 files, listed in `THEOREMS.md` section 0 item 3), which Lean adds to every declaration that mentions `M`. Each of the 26 declarations writes `[NeZero M]` as its first binder, with `omit [NeZero M] in` in front so the section instance is not duplicated (`notes/archive/F22_nezero.md` lists them; the `#check` output of the elaborated types is `notes/archive/audit_external_statement_nezero_check_2026-09-02.txt`) |
| `[NeZero S.card]`, `S.Nonempty` | `cor.2`, `prop:binarystacksvd_inadmissable` | the kept subset is nonempty |
| `hr : 0 < r` | the rank-`r` theorems that read a top-`r` eigenframe | the shared subspace is nontrivial; at `r = 0` both sides are `0`. It is absent from 8 declarations that never read it and present at the other 25 |
...
**7.20 One document in the packet's evidence trail is a snapshot and is stale in two places.**
`notes/archive/AUDIT_PACK_V4.md` is dated 2026-09-02 03:10. Its scope item S4 (rank-`r` `TableLawR`
assumed at `r_i ≥ 2`) and S10 (`thm:rank_r_stacksvd` has no Lean statement) are both closed:
`tableLawR_of_gaussian_rk` discharges `TableLawR` at general `r_i`, and
`thm_rank_r_stacksvd_gaussian` is the Lean statement (`TECHNICAL.md`). Items S1, S2, S6 to S9, S11, S12 and the caveats C1 to C21 are still current and
are the source of several rows above.

**7.21 One packaging defect.** Every Lean file header says "Released under Apache 2.0 license
as described in the file LICENSE", and the repository tracks a `LICENSE` file only under
`lean/StackedSVD/Vendor/COLT83/` (`git ls-files`). The project's own
LICENSE file is absent.

---

## 8. Remarks about the paper's statements

Four remarks compare the paper with the development. None of them reports a defect in a Lean
proof, and each names a Lean anchor, so the claim is checkable. The paper text quoted here is
that of the revised `main_paper.tex` (September 2026).

**`thm:gen_rank_weight_svdstack` assumes `β_ij > 0`, and Lean does not.** The paper states the
theorem under "`β_ij > 0` for all `i, j`" and keeps the footnote that puts aside the
weightings whose limit matrix has no eigengap; step 2 of its proof reads "`diag(β)` is
invertible by assumption". That hypothesis excludes every table that does not see every
component, which is the setting the unaligned model is built for. Lean needs neither the
hypothesis nor the footnote: the row-space bound at rank `r`, `‖V̂(W)ᵀV‖_F² ≤ tr(gᵀG⁻¹g)`,
holds at every `N` and uniformly over the weightings with `rank W ≥ r`; the identity
`tr(B_Rᵀ A⁻¹ B_R) = L⋆` holds at every rank of `B_R` (numeric agreement `1e-9`); and the bound
is attained at `W⋆ = D^{-1/2}`. The Lean statements carry `rank B_R = r` in place of
`β_ij > 0` (section 5), and the attainment without a rank hypothesis is the trace form
`thm_gen_rank_weight_svdstack_general_r_norank`. One numeric instance with `rank B_R = 1` at
`d = 400, 800, 1600` gave means `0.420, 0.402, 0.395` against `L⋆ = 0.400`. The
`rank B_R < r` half of the argument has no Lean twin. Anchors: `RankR/WeightedUpper.lean`,
`RankR/WeightedUpperG.lean` (`perfRGW_le_rowBoundRG`,
`thm_gen_rank_weight_svdstack_general_r_full`).

**Four minor items.** `thm:simple_thm1` is valid for every `M ≥ 1` and the two branches agree
at the threshold (anchor `thm_simple_thm1_svdstack_gaussian_full`); the strict clauses of
`thm:stacksvd_binary_optimal_svd_stack` and `prop:dominance` need no change, and are listed
only so the Lean strict wrappers have a paper location; the MLE appendix does not name the
Gaussian marginalization as a standard cited fact, although Lean proves that step (7.15); the
two remarks need no change.

**The distinctness clause of `assum:gen_rank_stacksvd_eig_sep` is needed for the value, not
only for the proof.** At a tied root `γ_ℓ = γ_m` the eigenvectors `z_ℓ, z_m` of
`prop:gen_rank_stacksvd_singleweight` are defined only up to a rotation inside a plane, and
the sum of reciprocals in the performance formula is not invariant under it. The proof's own
construction fixes the choice: the `z_ℓ` of a tied plane must be orthogonal for the form
`K(γ_ℓ) = ∑_i (w_i² / (w_i² - γ_ℓ)²) R_i Θ_i² R_iᵀ`. This is a remark, not an error, since the
assumption excludes ties. Evidence: closed form plus Monte Carlo
(`scripts/numeric/check_singleweight.py`, seed 20260905): on `M = 2`, `r = 2`, `θ² = (3, 15)`,
...
clause into a strict order and rules the tie out by hypothesis.

**The branch of the proof of `prop:singleweight_suboptimality` where the separation assumption
fails.** The paper says there only that the performance is bounded above by `β_0²`, with no
argument. Lean proves the exact limit in that branch, the summand of the larger weight in the
paper's own display, which is strictly below `β_0²`. This is a remark, not an error. Anchors:
`swLimitEx` (`RankR/SingleWeight/Example.lean`), `hlaw_one` and
`prop_singleweight_suboptimality_gaussian` (`RankR/SingleWeight/Suboptimality.lean`).
...
continue`). One binder is present in the elaborated type and absent from the source text of 21 quoted
rank-one declarations: the section-level instance `[NeZero M]` (7.19, `THEOREMS.md` section 0
...
on 2026-09-02: `lake env lean` exit 0, axioms `[propext, Classical.choice, Quot.sound]`.
...
source, and it covers the rank-`r` endpoint and the three satisfiability witnesses. Run it
...
come from StatsMLlib, not from this development.
...
| 2026-09-03 | The Gaussian marginalization behind `app:wstacksvd_mle`, adversarial audit of the seven new files, with a numeric check | pass: the random-effects law matches `SpikedModel.X`, the explicit square root and the density constant verified numerically (seed 20260903, `A Aᵀ = Σ` to `1e-16`, grid integral of the density `0.9999995`, clause 2 to `4e-15`), the theorem is not vacuous | 0 | `notes/archive/L5_mle_marginal.md` (section "Adversarial audit"), `notes/archive/audit_L5_numeric_2026-09-03.py` |
| 2026-09-02 | The declaration cleanup at commit `c858dba`, adversarial | loses no mathematics; 24 deletions all redirect to a survivor, 8 hypothesis drops sound | 3 (record and docstrings), 7 should-fix | `notes/archive/audit_cleanup_pass2_2026-09-02.md` |
| 2026-09-02 | The declaration cleanup at commit `9d727cc`, adversarial | loses no mathematics; 14 deletions, 5 dropped hypotheses sound | 3 (record and docstrings), 7 should-fix | `notes/archive/audit_cleanup_pass1_2026-09-02.md` |
...
| 2026-09-02 | The rank-`r` heteroscedastic chain (`RankR/Het/`), five files, independent | all five files pass; instantiated the final theorem on a concrete Gaussian model and matched `gammaR` to `theory_pred.R` to `1.0e-13` | 0, 9 should-fix (one a scope note) | `notes/archive/audit_independent_TrackE_wave2b_2026-09-02.md` |
| 2026-09-02 | The rank-`r` heteroscedastic chain, three earlier files, independent | all three files pass | 0, 5 should-fix | `notes/archive/audit_independent_TrackE_wave2a_2026-09-02.md` |
| 2026-09-02 | The rank-`r` heteroscedastic chain, six earlier files, independent | all six files pass | 0, 6 should-fix | `notes/archive/audit_independent_TrackE_wave1_2026-09-02.md` |
| 2026-09-02 | The plan of the rank-`r` heteroscedastic chain, before any proof | accept with corrections; the target statement was wrong as written and was restated | see note | `notes/archive/audit_rankr_plan_E_2026-09-02.md` |
| 2026-09-02 | The rank-`r` aligned appendix | pass with findings; 0 fatal | 0, 9 minor | `notes/archive/audit_independent_trackD_2026-09-02.md` |
| 2026-09-02 | Gaussian `TableLawR` at general `r_i` | pass with findings; 0 fatal | 0, 7 minor | `notes/archive/audit_independent_trackC_2026-09-02.md` |
| 2026-09-02 | Rank-`r` stacksvd for Gaussian noise | pass with findings; 0 fatal | 1 (scope documentation, since addressed), 4 minor | `notes/archive/audit_independent_trackA_2026-09-02.md` |
| 2026-09-02 | The plan of Gaussian `TableLawR`, before any proof | accept with corrections; one route dropped from 900 to 1400 lines to 200 to 350 | see note | `notes/archive/audit_rankr_plan_C_2026-09-02.md` |
| 2026-09-02 | The rank-one batch (the facades, `MLE.lean`, the remarks), independent | pass with findings; every theorem with more than two hypotheses instantiated on a concrete Gaussian model, so no hypothesis set is contradictory | 0, 5 minor | `notes/archive/audit_independent_D33_2026-09-02.md` |
| 2026-09-01 | General-`r_i` svdstack, independent | on track; 0 blockers | 0, 3 weakenings, 8 cosmetics | `notes/archive/audit_independent_trackB_2026-09-02.md` |
...
| 2026-09-01 | The rank-`r` stacksvd and svdstack plans, before any proof | both plans survive with corrections | see notes | `notes/archive/audit_rankr_plan_A_2026-09-01.md`, `..._B_2026-09-01.md` |
...
| 2026-09-01 | The Gaussian discharge of `HeteroLaw` (11 files, 11989 lines) | see note | see note | `notes/archive/audit_het_chain_2026-09-01.md` |
...
`notes/archive/AUDIT_PACK_V4.md` is the current one.

---

## 11. Decisions and open questions

**Decisions.** `notes/FLAGGED.md` is the register. Its table has 26
numbered rows, `D1` to `D24` plus `D26` and `D27`; there is no `D25`. Seven further blocks,
`D28` to `D34`, carry decisions in prose, and the `D32` block carries a numbered sub-list of
17 items, several with their own sub-decisions. No decision is left open.
...
| D12, D22 | weighted stackSVD proceeds over `HeteroLaw`, and `RMT/Het/` discharges it | the hypothesis is proved, by `heteroLaw_of_gaussian` |
| D13 | rank-`r` performance is `tr(Bᵀ specInvTop A r B)` | this is the trace functional of scope item 7.8 |
| D14, D16 | `Rank(∑ R_i R_iᵀ) = r` dropped from the model; `rank B_R = r` chosen as the replacement hypothesis after the weaker form was found false by counterexample | changes the hypothesis of `thm:gen_rank_weight_svdstack` |
...
| D-A1 (item 10) | the ordered-class narrowing is documented, and an order-free model stays a later option | scope items 7.2, 7.3 |
| D23 | the two appendix results of Appendix D are formalized after the main results | scope item 7.5 |
...
| Q3 | Layer 1 is Gaussian-only through `JointGaussianNoise`; `assum:general_noise` is wider | the rank-one SVDstack family takes `IndepNoise` and `thm_theta_est` takes `ThetaEstLaw`, so no rank-one Layer 1 theorem keeps `JointGaussianNoise`; `ThetaEstLaw` is also discharged for any fixed law with mean 0, variance 1 and finite fourth moment (`thetaEstLaw_of_general`, `thm_theta_est_general`, `notes/NONGAUSSIAN_SCOPE.md`), the first non-Gaussian discharge |
| Q4 | Prove a block-variance Marchenko-Pastur law, or stop at a second hypothesis structure | closed: `heteroLaw_of_gaussian` |
| Q5 | The `M = 3` remark example sat on the threshold | resolved: the revised paper takes `θ_3 = c_3^{1/4} + ε` |
...
| Q8 | The removal remark after `thm:gen_rank_weight_svdstack` is wrong in general | not formalized, scope item 7.5; resolved: deleted in the revised paper |
| Q9 | The paper wrote `prop:stacksvd_general` where it meant `prop:stacksvd_subspace` (the text after `prop:stacksvd_subspace`) | resolved: no label typo in the revised paper |
| Q10 | Four appendix results had no Lean | all four have Lean statements: `thm:rank_r_stacksvd` and `thm:rank_r_svdstack` are proved, and `prop:gen_rank_stacksvd_singleweight` and `prop:singleweight_suboptimality` have Layer 1 statements and Gaussian forms (sections 5, 7.5) |
...
| Q15 | The MLE and remark statements | closed: both statements stand |
| Q16 item 1 | Keep the ordered rank-`r` class, or add a permutation `R_i` route for the paper's worked example | **open**; the tree keeps the ordered class |
| Q16 item 2 | The component-clause statement form of `thm:rank_r_svdstack` (canonical frame, `hS : StrictAnti`) | **closed**: the component clause is stated in the canonical frame with `hS : StrictAnti` (`notes/FLAGGED.md` Q16) |

---

## 12. Repository map

From `TECHNICAL.md` (layout). Every folder has a README.

**Top-level files.** `README.md` (the plain-language entry point: a path per reader, the
result table, the caveats, how to check it, the folder map), `GETTING_STARTED.md` (the
install-and-check guide for a coauthor), `CITATION.cff`, `LICENSE`, and the four folders
`docs/`, `lean/`, `notes/`, `scripts/`. There is no continuous integration; the gates run by
hand on the server. **`docs/`** holds `TECHNICAL.md` (the
detailed paper-to-theorem table, the notation table, the full file tree and the verification
section), `THEOREMS.md` (the auditor annex), `docs/SORRIES.md` (the
`sorry` ledger, empty, read by `scripts/check_sorries.sh`), a `README.md` that names each,
and this file. Three further reader documents are in `notes/`:
`notes/paper_edits.md` (the exact old and new LaTeX of every paper edit, applied in the
paper's revision and cited by item number in the Lean docstrings),
`notes/NOTE_FOR_COAUTHORS.md` (one page) and
`notes/paper/main_paper.tex` (a read-only snapshot of the paper, taken 2026-08-29).
`notes/archive/PLAN_2026-08-29.md` is the first plan of record (the decision trail, not the
current state).

**`lean/`** is the lake project and `lean/StackedSVD/` is the source. Measured on the
tree of the build of record: **108508 lines in 210 files ours, plus 3716 vendored lines in 20
files** (`wc -l` over the tree, 2026-09-10 16:03 EDT, on the tree of commit `416ab9d`).

| Directory | Files | Content |
|---|---|---|
| root of the source | 16 | the rank-one model (`Defs.lean`), `Spectral.lean`, the hypothesis structures `RMT.lean` and `StackSVDWeighted.lean` (its Layer 1 consumer is in `StackSVD/Weighted.lean`), `StackSVD.lean` (likewise, in `StackSVD/Main.lean`), `Scalars.lean`, `Secular.lean`, `ThetaEst.lean`, `MLE.lean`, `MLEConverse.lean`, `Remarks.lean`, `RemarksUniform.lean`, `StrictFacades.lean`, `SVDStack.lean`, `Sat.lean`, `Existence.lean` (section 6) |
| `StackSVD/` | 2 | `Main.lean` (`prop:stacksvd_general`, `thm:simple_thm1` stacksvd half) and `Weighted.lean` (`cor.2` binary, `thm:stacksvd_weighted`, `thm:stacksvd_binary_optimal_svd_stack`), the Layer 1 theorems of `StackSVD.lean` and `StackSVDWeighted.lean` |
| `SVDStack/` | 11 | `A_β`, the svdstack estimator, `thm:svd_stack_general`, `thm:svdstack_weighted`, the uniform Rayleigh bound, `thm:simple_thm1`, the signed form of `lem:entrywise_conv_eigenvec` (`EntrywiseSigned.lean`), the svdstack clauses of `prop:binarystacksvd_inadmissable` (`Inad.lean`), the finite-`d` form of `lem:delocalization` (`DelocFinite.lean`); `DelocDir.lean`, the consumer-free half of `Gram.lean` |
| `LinAlg/` | 10 | Weyl, Davis-Kahan, top-`r` and index projector perturbation, Ky Fan, frames |
| `Prob/` | 6 | convergence in probability, polynomial null sets, Gaussian matrix laws and adapters; `WithDensityPi.lean` (densities of product measures and of linear images) and `GaussianDensity.lean` (the density of `N(0, S)` on `Fin d → ℝ`) |
...
| `RMT/General/Edge/` | 14 | the sharp upper edge of the noise Gram matrix at four moments by the moment method with a truncation (`notes/stage3_edge.md`), `opNorm_sq_edge_of_general`, `lamMax_W0_edge_of_general`, `singleTableLaw_of_moments`; the Furedi-Komlos shape count `cellCard_mul_le_pos` (`CountBound.lean`) |
| `RankR/` | 25 | Section 7 statements and Layer 1: the two models, `TableLawR`, `SubspaceLaw`, `HeteroLawR`, the four Section 7 results, asymptotic orthogonality of the svdstack columns (`AlignedOrth.lean`); `Unweighted.lean`, `WeightedMain.lean` and `SubspaceMain.lean` hold the Layer 1 theorems of `Defs.lean`, `Weighted.lean` and `Subspace.lean` |
| `RankR/RMT/` | 25 | Gaussian `TableLawR` at general `r_i` and Gaussian `SubspaceLawG` |
| `RankR/Het/` | 13 | `heteroLawR_of_gaussian`, `thm_rank_r_stacksvd_gaussian`; `Sub` (the sub-model that drops the zero-weight tables) |
| `RankR/SingleWeight/` | 9 | one weight per table (Appendix D): `SingleWeightLaw`, `prop_gen_rank_stacksvd_singleweight`, `prop_singleweight_suboptimality_of_law`, `prop_singleweight_suboptimality_gaussian` (section 6.8, 6.9; `Tie.lean`, `Regimes.lean` and `Suboptimality.lean` carry the discharge of `hlaw` on the witness) |
| `RankR/SingleWeight/Het/` | 8 | `singleWeightLaw_of_gaussian` and its facades |
...
**`scripts/`** holds the seven gates (`check_sorries.sh`, `check_axioms.sh` with its driver
...
`LayerAudit.lean`, section 4, the kernel gate `check_kernel.sh`, which replays every module through the kernel with the toolchain's `leanchecker`, and the core gate `check_core_imports.py`, which checks that the 92 modules of the Gaussian RMT core import no paper-specific module), the document path check `check_paths.py` (not a gate of record: every path the reader-facing documents name exists), the one-command header `make_audit_pack_header.sh` that runs
...
scripts of `scripts/numeric/`, and `scripts/README.md` documenting all of them.
...
`FLAGGED.md` (decisions and questions), `INTERFACES.md` (the fixed names the notes use),
`README.md` (every paper label with its formalization class A, B or C, and the translation of
the paths that archived material still uses),
`FOLLOWUP_LIST.md` and `REVISION_LIST.md` (the follow-up items and the paper revision items),
`RANK_R_PLAN.md`, `NONGAUSSIAN_SCOPE.md` and `stage3_edge.md` (the non-Gaussian
extension and its deferred edge stage), `REPO_CLEANUP_PLAN.md`,
`BUILD_HISTORY.md` (every earlier build of record), `SERVER_SETUP.md` (the server and laptop
setup), `discoveries/` and `audit_packets/` (each with a README).
`notes/archive/` holds completed material: per-theorem review notes, earlier audits and
audit packs, external reviews, campaign plans, and `agent_reports/` (197 files), listed in
its `INDEX.md`.
...
state, the toolchain, the full build and four of the seven gates; the fifth,
`scripts/check_layering.sh`, the sixth, `scripts/check_kernel.sh`, and the seventh,
`scripts/check_core_imports.py`, are run separately (section 9, their rows). The script always exits 0 by design, so read the
...
NO VERBATIM MATCH: thm_gen_rank_weight_svdstack_general_r_paper_gaussian  in: ['RankR/GeneralGaussian.lean']
NO VERBATIM MATCH: thm_gen_rank_weight_svdstack_general_r_full_gaussian  in: ['RankR/GeneralGaussian.lean']
