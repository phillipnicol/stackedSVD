...
V1 <- V[,1]
X1 <- theta * u1 %*% t(V1) + matrix(rnorm(n*d, sd=1/sqrt(d)), n, d)

V2 <- sin(psi) * V[,1] + cos(psi) * V[,2]
X2 <- theta * u2 %*% t(V2) + matrix(rnorm(n*d, sd=1/sqrt(d)), n, d)
...
write.csv(results, file = "../data/unaligned_simulation_results_theta_3.csv", row.names = FALSE)

results <- read.csv("../data/unaligned_simulation_results_theta_3.csv")
...
print(p)
ggsave("../plots/unaligned_simulation_comparison_theta_3.pdf", p, width = 10, height = 6, dpi = 300)
