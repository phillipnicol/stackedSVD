...
/-!
# `thm:gen_rank_weight_svdstack`: the weighted rank-`r` svdstack limit

Moved out of `RankR/Weighted.lean` (F28, 2026-09-08):
`thm_gen_rank_weight_svdstack_general` through `_max_gaussian`, the Layer 1 and
...
/-- `thm:gen_rank_weight_svdstack` for one admissible weight matrix `W`, at `r_i = 1`. Same
...
theorem thm_gen_rank_weight_svdstack_general (m : UnalignedModel μ M n d r) (c β : Fin M → ℝ)
...
/-- **`thm:gen_rank_weight_svdstack`** at `r_i = 1`: with the weights
...
paper's claim is `thm_gen_rank_weight_svdstack_opt`, and the two are put together in
`thm_gen_rank_weight_svdstack_max`.

The hypothesis is `rank B_R = r`, which is what the eigengap at `W⋆` needs. The paper's own
`β_ij > 0` plus `Rank(∑ R_i R_iᵀ) = r` implies it (`thm_gen_rank_weight_svdstack_paper`) and is
...
theorem thm_gen_rank_weight_svdstack (m : UnalignedModel μ M n d r) (c β : Fin M → ℝ)
  ...
  have h := m.thm_gen_rank_weight_svdstack_general c β (optWR β) hβdef hr hrM hgap hpos law hG
  rwa [limitRW_optWR β m.R h0 h1 hrp] at h

/-- `thm:gen_rank_weight_svdstack` with the paper's own hypotheses: `β_ij > 0` for every `i, j`
and the rank condition of `assum:unaligned`. -/
theorem thm_gen_rank_weight_svdstack_paper (m : UnalignedModel μ M n d r) (c β : Fin M → ℝ)
    (hc : ∀ i, 0 < c i) (hβdef : ∀ i, β i = beta (m.tbl i).θ (c i))
  ...
  m.thm_gen_rank_weight_svdstack c β hc hβdef hrM
    (rank_BR_of_ne_zero m.R (fun i => (hβpos i).ne') hrank) law hG

/-- **The optimality half of `thm:gen_rank_weight_svdstack`**: no admissible weight matrix beats
...
theorem thm_gen_rank_weight_svdstack_opt (m : UnalignedModel μ M n d r) (c β : Fin M → ℝ)
...
/-- **`thm:gen_rank_weight_svdstack`, both halves, in one declaration.** The performance of
...
(`thm:gen_rank_weight_svdstack`, which also defines `W\opt`).
...
performance at `W`; that link sat in `thm_gen_rank_weight_svdstack_general` and needed `law`
and `hG`, which this theorem already carries
(`notes/archive/audit_rank_r_weighted_post_2026-08-30.md`, finding 4.1). -/
theorem thm_gen_rank_weight_svdstack_max (m : UnalignedModel μ M n d r) (c β : Fin M → ℝ)
    (hc : ∀ i, 0 < c i) (hβdef : ∀ i, β i = beta (m.tbl i).θ (c i))
    (hr : 0 < r) (hrM : r ≤ M)
    (hrankB : (BR β m.R).rank = r)
    (law : ∀ i, (m.tbl i).SingleTableLaw (c i)) (hG : m.JointGaussianNoise) :
  ...
  ⟨m.thm_gen_rank_weight_svdstack c β hc hβdef hrM hrankB law hG,
    fun W hgapW hposW =>
...
`thm_gen_rank_weight_svdstack` and `thm_gen_rank_weight_svdstack_max` take
...
`thm_gen_rank_weight_svdstack`. The `∀ W` half keeps the two admissibility hypotheses,
because an arbitrary weight matrix can be singular. -/

/-- **`thm:gen_rank_weight_svdstack`** at `r_i = 1`, Layer 2 form: the
same conclusion as `thm_gen_rank_weight_svdstack` from the proportional regime of each table
and the joint Gaussian law, with no `SingleTableLaw` hypothesis. -/
theorem thm_gen_rank_weight_svdstack_gaussian [∀ N, IsProbabilityMeasure (μ N)]
    ...
    TendstoInProb μ (fun N ω => m.perfRW (optWR β) N ω)
      (limitROpt β m.R (by simpa using hrM)) :=
  m.thm_gen_rank_weight_svdstack c β hc hβdef hrM hrankB
    (fun i => SpikedModel.singleTableLaw_of_gaussian (hc i) (m.tbl i) (hreg i)
      (m.gaussianNoise_of_joint hG i)) hG

/-- **`thm:gen_rank_weight_svdstack`, both halves, Layer 2 form.** The performance of weighted
...
law. Same statement as `thm_gen_rank_weight_svdstack_max` with `SingleTableLaw` discharged. -/
theorem thm_gen_rank_weight_svdstack_max_gaussian [∀ N, IsProbabilityMeasure (μ N)]
    (m : UnalignedModel μ M n d r) (c β : Fin M → ℝ)
    (hc : ∀ i, 0 < c i) (hβdef : ∀ i, β i = beta (m.tbl i).θ (c i))
    (hr : 0 < r) (hrM : r ≤ M)
    (hrankB : (BR β m.R).rank = r)
    ...
    TendstoInProb μ (fun N ω => m.perfRW (optWR β) N ω)
      ...
      ∀ W : Matrix (Fin M) (Fin M) ℝ,
        ...
        TendstoInProb μ (fun N ω => m.perfRW W N ω) (limitRW W β m.R) ∧
          limitRW W β m.R ≤ limitROpt β m.R (by simpa using hrM) :=
  ⟨m.thm_gen_rank_weight_svdstack_gaussian c β hc hβdef hrM hrankB hreg hG,
    fun W hgapW hposW =>
      ⟨m.thm_gen_rank_weight_svdstack_general c β W hβdef hr hrM hgapW hposW
        (fun i => SpikedModel.singleTableLaw_of_gaussian (hc i) (m.tbl i) (hreg i)
          (m.gaussianNoise_of_joint hG i)) hG,
        m.thm_gen_rank_weight_svdstack_opt c β hc hβdef hr hrM W hgapW hposW⟩⟩
