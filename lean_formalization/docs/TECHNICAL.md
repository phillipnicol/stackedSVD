...
> Location: this file lives in `docs/`. The bare names `TECHNICAL.md`, `THEOREMS.md` and
> `AUDIT_DOC.md` refer to the files of this folder; every other path is relative to the
> repository root. The short `README.md` at the root is the plain-language entry point.
...
**no open proof**, and it contains no `axiom`. Every public declaration depends only on
`propext`, `Classical.choice` and `Quot.sound`; no declaration depends on `sorryAx`. Gate 2 (`scripts/check_axioms.sh`)
...
header-linter lines. Seven gates on that tree, each exit 0 (the six quick ones at 16:02
EDT, the kernel gate at 16:04:23 to 16:15:50 EDT): `check_sorries.sh` 0 sorry sites in 0
...
forbidden hits; `check_core_imports.py` (v2) 95 core modules (75 ours, 42494 lines), no import outside the
...
`scripts/check_paths.py` (not a gate of record): 2087 paths resolve, 0 missing (2026-09-10).
Earlier builds of record: `notes/BUILD_HISTORY.md`.
...
they drift). `m` is the model, `N` the index of the sequence, `ω` the sample point.
...
Five rows carry one (stages 0 and 1 of `notes/NONGAUSSIAN_SCOPE.md`); `THEOREMS.md`
sections 2.1 and 3.1 give both binders and the route.

| Paper label | Gaussian theorem | Layer 1 theorem | File | Scope note |
|---|---|---|---|---|
| `prop:single_table` | `singleTableLaw_of_gaussian`; `singleTableLaw_of_general` (any fixed noise law with mean 0, variance 1, a finite fourth moment and a Lebesgue density, given the upper edge of the bulk) | `SpikedModel.SingleTableLaw` (structure) | `RMT/Full.lean`, `RMT.lean`, `RMT/General/Sup.lean` | both regimes; `topSimple` added; the general form is an implication, the edge is a hypothesis |
| `lem:delocalization` | via `lem_delocalization` + `singleTableLaw_of_gaussian` (`RMT/Full.lean`); `lem_delocalization_of_general` (general noise, one edge hypothesis per table) | `lem_delocalization` | `SVDStack/Gram.lean`, `General/Layer1.lean` | signed limit `⟪v̂_i, v̂_j⟫ → β_iβ_j` |
...
| `prop:stacksvd_general` | `prop_stacksvd_general_gaussian`, `_inner_gaussian`; `prop_stacksvd_general_of_general` (general noise, one edge hypothesis at the stack) | `prop_stacksvd_general`, `_inner` | `StackSVD/Main.lean`, `General/Layer1.lean` | reduces to the stack at `(‖θ‖₂, ‖c‖₁)`; `stack_law_general` keeps that reduction at a general law |
| `thm:svd_stack_general` | `thm_svd_stack_general_gaussian`, `thm_svd_stack_general_inner_gaussian`, `thm_svd_stack_general_zero_gaussian`; `thm_svd_stack_general_of_general` (general noise, one edge hypothesis per table) | `thm_svd_stack_general`, `_inner`, `_zero` | `SVDStack/Main.lean`, `General/Layer1.lean` | `β_2 > 0` unsorted; the general form covers the main clause, not `_zero` |
...
| `thm:theta_est` | `thm_theta_est_gaussian`; `thm_theta_est_general` (any fixed noise law with mean 0, variance 1, finite fourth moment; the single-table law of table `i` stays a hypothesis); `thm_theta_est_of_general` (that law discharged too, at the price of a density and the edge of table `i`) | `thm_theta_est` | `ThetaEst.lean`, `General/Layer1.lean` | item P proved, not assumed; `ThetaEstLaw` also discharged at `NoiseLaw` (`thetaEstLaw_of_general`) |
...
| `remark:svd_outperform_stack` | `remark_svd_outperform_stack_exists` (model built, `θ = (√5, 4)`, `c = (1, 38.4)`), `remark_svd_outperform_stack_two`, `_pair_*`, `_three_*` | instances of the closed forms | `Existence.lean`, `Remarks.lean`, `RemarksUniform.lean` | the `M = 3` example is on the threshold |
...
| `thm:gen_rank_weight_svdstack` | `thm_gen_rank_weight_svdstack_general_r_paper_gaussian`, `thm_gen_rank_weight_svdstack_general_r_full_gaussian`, `thm_gen_rank_weight_svdstack_gaussian` | `thm_gen_rank_weight_svdstack`, `_opt`, `_max`, `_paper`, `_general_r`, `_full` | `RankR/GeneralGaussian.lean`, `RankR/WeightedMain.lean` | `rank B_R = r` in place of the paper's `β_ij > 0`, and no eigengap of `C`; uniform-in-`W` bound |
...
| `thm:rank_r_svdstack` | `thm_rank_r_svdstack_aggregate_gaussian`, `thm_rank_r_svdstack_component_gaussian`, `_of_model_gaussian` | `thm_rank_r_svdstack_aggregate`, `_component`, `_component_eig` | `RankR/GeneralGaussian.lean`, `RankR/WeightedUpperG.lean`, `RankR/AlignedMain.lean` | both clauses; canonical frame |
...
paper's own mode (Section `sec:model_setup` defines `\pto` that way). Overlaps are
...
`hθanti`); the last spike may be zero.

**The reusable core.** The 95 modules under `singleTableLaw_of_gaussian` and its
general-noise twin `singleTableLaw_of_general` (`Defs`, `Spectral`, `RMT`,
the 19 files of `RMT/` outside `RMT/Het/` and `RMT/General/`, the 22 of `RMT/General/`, the
14 of `RMT/General/Edge/`, the 11 of `Prob/`, `LinAlg/Eigen`, `Frame`, `KyFan`,
`SpecInvReindex`, `SpecProjPerturb`, `TopProjPerturb`, and the 20 vendored files of
`Vendor/COLT83/`; 75 files and 42494 lines ours) import no module that mentions stacking,
weights or the paper's estimators. `scripts/check_core_imports.py` (gate 7) keeps it that
way, and `README.md` ("The Gaussian RMT core") describes the entry theorem in words. The other three Gaussian discharge families import the paper's definition
...
prints that list. The Layer 1 theorems sit in six files above those definition files
(`StackSVD/Main.lean`,
`StackSVD/Weighted.lean`, `RankR/Unweighted.lean`, `RankR/WeightedMain.lean`,
`RankR/SubspaceMain.lean`; `SVDStack/DelocDir.lean` holds the consumer-free content
of `SVDStack/Gram.lean`), so a discharge family's imports carry no Layer 1
...
need. EXT holds 177 modules, is import-closed, and no declaration inside it takes one of
...
`scripts/check_core_imports.py` (gate 7) checks the core, EXT, and that five entry
theorems exist. `SVDStack/Defs.lean` has no consumer of any of the eight structures, so
`vMax` belongs to EXT.
...
`thm_theta_est` reads the noise through `MultiTableModel.ThetaEstLaw`), so a four-moment
discharge of the structures would give every result
unchanged. Stage 1 is that discharge for `SingleTableLaw`
...
Lebesgue density, which excludes Rademacher entries. Stage 3 proves
...
Stage 3 chain, the Furedi-Komlos count, is proved as `cellCard_mul_le_pos` in
`RMT/General/Edge/CountBound.lean`; `docs/SORRIES.md` has no row.
...
remark after `thm:gen_rank_weight_svdstack`, which does not hold in general. One more
appendix result, `prop:singleweight_suboptimality`, needs one extra step. The Gaussian
discharge of `SingleWeightLaw` needs the separation assumption `EigSep`, which fails on its
two-table witness at extreme weight ratios. `RankR/SingleWeight/Suboptimality.lean` proves
the hypothesis `hlaw`, the single-weight limit on that witness at every positive weight
pair, regime by regime, so the proposition is unconditional for Gaussian noise. In the
rank-`r` results the spike order inside a table is a model field, so a table with a repeated
`θ_ij` is outside the class where the paper allows it. See `THEOREMS.md` section 7 for the full
list, including the well-formedness hypotheses that remain.
...
The kernel gate (`check_kernel.sh`) is the second pass of the kernel: the
...
the models. `StackedSVD/Existence.lean` uses the same constructor to prove the
...
hypothesis). The file layout shows the split too: every Layer 1 consumer of a discharge
family's definition file sits in a file above it (`StackSVDWeighted.lean`'s consumer is in
`StackSVD/Weighted.lean`). Three `Sup` files still mix a discharge and a consumer on
...
`notes/`, so without the paper snapshot and the two paper-facing documents it holds;
`scripts/release_withheld.txt` lists every withheld path.
...
GETTING_STARTED.md      install Lean, build the project, check one theorem
LICENSE (Apache 2.0), CITATION.cff (the paper and the repository)
docs/                   the detailed documents:
  ...
  TECHNICAL.md          the detailed map: the notation table (paper symbol to Lean name),
                        main-results table, the two layers, verification, layout, build of
                        record
  ...
  SORRIES.md            the ledger of open proofs; the table is empty, and gate 1 keeps the
                        ledger and the tree in step
notes/                  the working documents, withheld from this copy: README (the inventory
                        of paper results and their formalization class), SERVER_SETUP (the
                        toolchain of the development server and of the laptop),
                        NONGAUSSIAN_SCOPE (the stages of the non-Gaussian extension),
                        stage3_edge (the Bai-Yin edge plan), STAGE3_CAMPAIGN (the plan and
                        the fallback route of Stage 3), x8_plan (the proof plan of the Furedi-Komlos
                        count), REPO_CLEANUP_PLAN, RELEASE_PLAN,
                        BUILD_HISTORY (every earlier build of record)
notes/archive/          completed material: per-theorem review notes, audits, packets,
                        external reviews and responses; INDEX.md lists every file
notes/paper/main_paper.tex  read-only snapshot of the paper; never edited
notes/paper_edits.md   the exact old and new LaTeX of each paper edit, with its Lean anchor;
                        the paper's revision applies them, and the Lean docstrings cite its
                        items by number
notes/NOTE_FOR_COAUTHORS.md  one page for a coauthor: proved, broke and changed, left
...
notes/discoveries/      short technical records; README.md lists them
scripts/                README.md (one entry per script), check_sorries.sh, check_axioms.sh, check_root_imports.py,
                        check_theorems_sigs.py, check_layering.sh,
                        check_kernel.sh (gate 6: leanchecker replays every
                        module through the kernel, about 10 min and 15 GB, on the root-disk
                        olean copy after a cmp against the build; on a machine without the
                        nprocs shim it runs KernelReplay.lean
                        ...
                        check_core_imports.py (gate 7: the 95 modules of the Gaussian RMT
                        core import no paper-specific
                        module; a wider set, EXT (177 modules: the core, the four Gaussian
                        ...
                        the numeric check of the three weight regimes of the single-weight
                        witness (one weight per table);
                        check_paths.py (not a gate of record: every path the
                        reader-facing documents name exists; it reads
                        scripts/release_withheld.txt in the public copy);
                        release/make_release.py (exports the public
                        copy, the lean_formalization/ folder of the paper's code repository,
                        notes/RELEASE_PLAN.md); scripts/numeric/ (exploration scripts, with
                        their own README)
lean/                   the lake project (Mathlib v4.33.0 + StatsMLlib 37286c3); the sources
                        are lean/StackedSVD/,
                        108508 lines ours in 210 files, 3716 vendored in 20; every path
                        below is relative to lean/StackedSVD/

  Main.lean             the statement file: 40 theorems, one per paper result
  ...
  StackSVD.lean         stacking lemmas, the stack estimator (its Layer 1 theorems are in
                        StackSVD/Main.lean)
  StackSVDWeighted.lean HeteroLaw, secular scalars, optimal weights (its Layer 1 theorems
                        are in StackSVD/Weighted.lean)
  ...
  SVDStack/Gram.lean            align, gram, item D lem:delocalization (the consumer-free
                                half is in SVDStack/DelocDir.lean)
  ...
  Prob/WithDensityPi.lean       densities of product measures and of linear images
  Prob/GaussianDensity.lean     the density of N(0, S) on Fin d → ℝ, image of the standard
                                Gaussian under a matrix, its charFun
  ...
  RMT/Sup.lean, TailShift.lean  the Layer 2 assembly and the index shift that drops hn2
  RMT/Full.lean                 singleTableLaw_of_gaussian, both regimes
  RMT/General/                  Stage 1, the non-Gaussian single table (notes/archive/
                                prop_single_table_general.md): Defs (the model
                                definitions of the stage and the structure
                                ...
                                (align_tendstoInProb_of_subcritical_general'), DelocUniform
                                (delocUniform_of_lamMax: the window bound at w ⊥ v),
  ...
  RMT/General/Edge/             Stage 3, the sharp upper edge at a general law (route C of
    ...
    Arith.lean                  1 <= k_N and 8 k_N <= d_N for N large,
                                fkFactor at most 1/2 for N large, and the Markov limit at
                                C = markovConst c eps
    Trunc.lean                  the split Z = Zhat + R + m_T J at the level
                                d^(1/2 - a), truncLaw is a TruncNoiseLaw, the mean matrix
                                has small operator norm
    Sparse.lean                 with probability tending to 1 the discarded part R
                                has operator norm at most eps sqrt d (no large entry, and
                                no row or column with two of them)
    Trace.lean                  trace ((Y^T Y)^k) as a sum over the closed walks of
                                length 2k on Fin n x Fin d, its integral as a product of
                                entry moments, and the k = 2 closed form as the check
    Compare.lean                the three walk classes; the trace at a
                                TruncNoiseLaw is at most the Gaussian trace plus excessSum
    Gaussian.lean               the Gaussian trace bound at k = momOrder C d, from
                                R3.integral_opNorm_le and R3.measure_opNorm_ge_le
    Count.lean                  the walk classes and the vertex count
                                (walkVerts, clsCard, cellCard); 536 lines
    Code.lean                   the six-part step code of a walk (code,
    ...
    Dyck.lean                   the contour walk of a Dyck word
                                ...
                                lines
    CountBound.lean             the assembly: the three arithmetic
                                lemmas (descFac_mul_min_pow_le, choose_mul_le_pow_choose,
                                code_pow_arith), cellCard_mul_le_pos (the Furedi-Komlos
                                count itself), cellCard_mul_le and
                                clsCard_mul_le (294 lines)
    Excess.lean                 excessSum at most 2 fkFactor times the Gaussian
                                trace, relative to the tree-walk class
    Markov.lean                 the two spectral
  ...
  General/Layer1.lean           the Layer 1 half of Stage 1, outside RMT/ (gate 7):
  ...
  RMT/Het/                      Gaussian discharge of HeteroLaw:
  ...
  ThetaEst.lean         thm:theta_est over SingleTableLaw and ThetaEstLaw, item P
                        (noise_projection_tendsto), thetaEstLaw_of_gaussian;
                        also thetaEstLaw_of_general and thm_theta_est_general
  ...
  MLEMarginal/                  app:wstacksvd_mle, the Gaussian marginalization:
  ...
  SVDStack/Rayleigh.lean        the bound uniform in w
  SVDStack/EntrywiseSigned.lean lem:entrywise_conv_eigenvec in the paper's signed form
  SVDStack/Inad.lean            prop:binarystacksvd_inadmissable, svdstack clauses
  SVDStack/Simple.lean          thm:simple_thm1, svdstack half, every M >= 1
  SVDStack/DelocFinite.lean     finite-d lem:delocalization tail bound 1/(eps^2(d-1))
  ...
  RankR/                        Section 7 and Appendix E. Defs (r_i = 1 model), Unweighted
                                (its Layer 1 theorems), General (general r_i
                                model, SpikedModelR, UnalignedModelR, TableLawR, rk_le_d,
                                norm_colVecG), GeneralMain,
                                GeneralFrob, GeneralGaussian, Weighted, WeightedMain (its
                                Layer 1 theorems), WeightedUpper,
                                WeightedUpperG (thm:rank_r_svdstack aggregate), Subspace,
                                SubspaceMain (the Layer 1 theorems of Subspace and
                                SubspaceG), SubspaceG, SubspaceGStack, SubspaceGaussian,
                                Aligned,
                                AlignedComponent, AlignedMain (thm:rank_r_svdstack
                                component), AlignedOrth (svdstack columns orthogonal),
                                StackGamma (HeteroLawR), StackMain
  ...
  RankR/SingleWeight/           one weight per table (Appendix): Scalars (secMat,
                                ...
                                The discharge of hlaw on the witness:
  ...
  RankR/SingleWeight/Het/       the Gaussian discharge of SingleWeightLaw at general
                                R_i: Scalars (swFmat, swRho, swNu, the
                                ...
                                (the drop of the zero-weight tables, subRk, wSuppEmb,
                                ...
                                paper). The one-detectable-root regime
...
us. The tree has no open proof (`docs/SORRIES.md` has no row); every declaration is proved
with the three standard axioms.

`notes/SERVER_SETUP.md` records the toolchain setup of the development server and of the
laptop; it is not part of this copy.
